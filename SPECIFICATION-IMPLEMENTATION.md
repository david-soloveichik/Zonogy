...
- The zone click tap likewise answers on a thread of its own, separate from the keyboard taps' because its answer can involve a wait. A click can only be Zonogy's while CmdTab is showing or the gesture modifiers are held; every other click, which is nearly all of them, passes at once without involving the main thread. A click that may be Zonogy's is decided on the main thread, and the tap thread waits for the answer, because it depends on where the click landed: zone frames, zone bar hit areas, and Zonogy's own window frames live there.
- The Dock taps stay on the main run loop. But we try to make them enabled only while a click or drag could concern Zonogy; see the Click Interception and Dock Icon Drag Interception notes in `SPECIFICATION-DOCKMENUS.md`.
