...
Each selection carries the trail of moves that produced it. A press exactly opposite to the trail's last move pops back to that move's source, so any path retraces step by step to the gesture's start. (Note that moves are not symmetric — a full-height column reached from a stack's bottom re-enters the stack at its top — which is why reversal is remembered rather than recomputed.) A letter jump starts a fresh trail.
