...
Zone navigation selects structurally within a screen and geometrically across screens.

On one screen, moves are read from the zone model — each tiling zone's column side and stack row, with the floating zone bar as the bottom-most stop below both columns. Crossing columns lands in the matching row: top to top, bottom to bottom, and a full-height column enters a stack at its top zone. Off the floating zone bar, up climbs into the bottom row (the lower zone index wins between two bottom zones) and a horizontal press lands on the bottom-most zone of the column in that direction.

A press past the edge of the layout exits through `DirectionalRectNavigation`: a race among the other screens' zones and bars for the nearest rectangle ahead in the pressed direction, preferring one that overlaps the source along the perpendicular edge and falling back to center distance so diagonally-placed displays stay reachable. Two bar rules shape the race: horizontal races run among tiling zones only (a bar can overlap the zone frames and would win outright), and an upward winner is replaced by the entered screen's bar when that bar lies ahead (a downward exit starts from the bar itself, so it has already made that stop).
