...
The candidates are the zones themselves: every tiling zone by its zone frame, plus each screen's floating zone at its bottom-edge bar. The main specification's bar rules do not emerge from this geometry on their own, but need to be imposed specifically: (a) In a horizontal press, bars are removed from the candidates (a bar can overlap the zone frames and would win outright). (b) After a vertical press, a winner on another screen is replaced by the bar on the crossed screen boundary (otherwise, the selection could skip past).

Each selection carries the trail of moves that produced it. A press exactly opposite to the trail's last move pops back to that move's source instead of running the geometry, so any path retraces step by step to the gesture's start. Nearest-ahead selection is not symmetric — left then right can land on a third zone — which is why reversal is remembered rather than recomputed.

Because commits happen on modifier release, the gesture runs through a keyboard event tap rather than ordinary hotkeys. Arrow keys always move the selection — the keyset preference can add a letter cluster (HJKL, WASD, or IJKL) — and Return moves the focused window. The held modifier combination is configurable (at least two modifiers, like the mouse-gesture modifiers). The in-gesture Launcher key is borrowed from the Show Launcher shortcut's binding. The gesture snapshots zone geometry when it engages; commits re-check live occupancy. Anything that could invalidate the snapshot mid-gesture (another Zonogy shortcut firing, or a display change) cancels the gesture rather than letting a commit act on stale zones.
