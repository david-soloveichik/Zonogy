...
Zone navigation selects structurally within a screen; across screens, geometry picks the screen and the entry into it is structural.

On one screen, moves are read from the zone model — each tiling zone's column side and stack row, with the floating zone bar as the bottom-most stop below both columns. Crossing columns lands in the matching row: top to top, bottom to bottom, and a full-height column enters a stack at the stack's top zone. Off the floating zone bar, up climbs to the zone nearest the bar (a stacked bottom zone before a full-height column, then the lower zone index) and a horizontal press lands on the bottom-most zone of the column in that direction.

A press past the edge of the layout crosses to another screen. Each screen lies in exactly one direction from the current one: with overlapping x-ranges it is above or below, with overlapping y-ranges left or right. The press moves to the nearest screen lying in the pressed direction (by center distance along that axis, then across it) and enters through its near side: from below, the floating zone bar; from above, the zone nearest the top edge (a stacked top zone before a full-height column); from the side, the near column at the matching row, with a bar entering at that column's bottom-most zone.

Each selection carries the trail of moves that produced it. A press exactly opposite to the trail's last move pops back to that move's source, so any path retraces step by step to the gesture's start. (Note that moves are not symmetric — a full-height column reached from a stack's bottom re-enters the stack at its top — which is why reversal is remembered rather than recomputed.)

Because commits happen on modifier release, the gesture runs through a keyboard event tap rather than ordinary hotkeys. Arrow keys always move the selection — the keyset preference can add a letter cluster (HJKL, WASD, or IJKL) — and Return moves the focused window. The held modifier combination is configurable (at least two modifiers, like the mouse-gesture modifiers). The in-gesture Launcher key is borrowed from the Show Launcher shortcut's binding. The gesture snapshots the zones when it engages; commits re-check live occupancy. Anything that could invalidate the snapshot mid-gesture (another Zonogy shortcut firing, or a display change) cancels the gesture rather than letting a commit act on stale zones.
...
Some full-screen exits emit no resize event, which would leave a screen stuck in pause after its full-screen window is gone. Focus acts as the repair: when the user focuses a (non-full-screen) window on a paused screen, the screen is clearly no longer showing full-screen content, so the pause is lifted. The exception is a full-screen window whose Space is merely inactive behind the current one: the user never exited full-screen, and the screen goes through "Returning to the full-screen Space" (below), which lifting the pause would derail.
...
**Returning to the full-screen Space:** When a screen has a native full-screen window, MacOS creates a new Space for it on that screen. Although Zonogy's pipelines try to target another screen, we can't completely stop windows opening in the screen that's in full-screen mode. If a window opens on that screen, MacOS switches Spaces. This is undesired--instead we want to place any managed window opened in that way into a zone in another (non-full-screen) screen and go back to the full-screen Space so we are not interrupted. (For example, we are watching a movie in full-screen, and doing other things at the same time on another screen.) To get this behavior, we monitor Spaces through another private API (see "CGS Spaces membership query" below).
