...
- **Native-tab close-rebind:** When a single-application signal flags a placed window as gone — an `AXUIElementDestroyed` notification or a per-PID validation pass — Zonogy first attempts the native-tab close-rebind (see `SPECIFICATION.md`), keeping the window in its zone if a surviving sibling matches and pruning it only otherwise. The all-window `syncWindowsToZones()` sweep skips the rebind when it does not find a tracked window in WindowServer. This is because the global `CGWindowListCopyWindowInfo` snapshot can be transiently incomplete during wake or display changes, so the absence is not reliable enough to choose a sibling tab. The window falls through to deferred pruning (below), which can restore the `(pid, CGWindowID)` if it reappears.
