...
- **(Per-window) AX frame application retries:** `WindowController` schedules a one-shot delayed frame retry for a window when zone-frame application via AX fails (`Failed to set frame for window`) or when the post-apply result remains off-screen/far from the requested geometry. These per-window timers are cancelled whenever zone topology/geometry changes or when screens go to sleep so no stale frame targets are applied later.
