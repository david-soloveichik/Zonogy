...
The candidates are the zones themselves: every tiling zone by its zone frame, plus each screen's floating zone, represented by its occupant window's actual rectangle when filled or by the bottom-edge bar when empty.
...
Because commits happen on modifier release, the gesture runs through a keyboard event tap rather than ordinary hotkeys. The four directions and the Return move key therefore share one modifier combination: editing one member's modifier in Preferences re-syncs the others, and a modifier-free binding is not accepted for them. The in-gesture Launcher key is borrowed from the Show Launcher shortcut's binding. The gesture snapshots zone geometry when it engages; commits re-check live occupancy. Anything that could invalidate the snapshot mid-gesture (another Zonogy shortcut firing, or a display change) cancels the gesture rather than letting a commit act on stale zones.
