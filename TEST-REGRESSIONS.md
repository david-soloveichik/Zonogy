...
- Bug report: For an active right-column window, Sticky Resize's focus restore and ActiveFit both reposition it on focus/main-window notifications but to different frames (Sticky Resize to the rest origin, ActiveFit to the shifted reveal origin), so they fight. This has surfaced as the window hanging off-screen at the rest position while ActiveFit still believes it is revealed, and as visible thrashing between the two positions when an app emits a burst of such notifications (e.g. Messages on unminimize).
  - Think about: While a right-column window is in reveal mode, ActiveFit owns its frame (it already applies the remembered sticky size as its reveal candidate). Sticky Resize must defer to ActiveFit rather than undo the reveal, and ActiveFit must reassert the reveal when the window's actual frame no longer matches its cached reveal state instead of trusting that state.
