...
Tracking is per display, and the settle delay restarts on each occupancy change so only the arrangement that ultimately settles is captured. Displays in a full-screen Space are not tracked. A display entering native full screen captures its arrangement first, so the arrangement from before full screen is saved.
...
- On a display paused for a native full-screen window (see **Full-screen pause** in [SPECIFICATION.md](SPECIFICATION.md)), the chooser still opens. It pre-selects the most recent saved arrangement (the idea is that releasing the shortcut returns to the arrangement from just before full screen). Choosing an arrangement takes the window out of full-screen mode, and opens the arrangement; the former full-screen window is placed if the arrangement includes it, minimized otherwise. On a display paused for non-native full screen there is nothing to leave, so the chooser does not open.
