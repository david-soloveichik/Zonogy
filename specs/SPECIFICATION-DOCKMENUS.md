...
- **Click interception:** Clicking an application icon in the Dock (without Shift) does not activate the real Dock item; instead Zonogy performs the same default action as selecting that application in the Launcher (see [SPECIFICATION-LAUNCHER.md](SPECIFICATION-LAUNCHER.md)). In particular, it obeys `hasMainWindow` selection rules. For non-running apps or running apps with no managed windows, Zonogy simulates a press on the Dock item (via Accessibility API) to trigger the app's native "clicked in Dock" behavior.
  ...
  - **Exception compared to Launcher**: While the Launcher allows "moving" a currently open window from one zone to another, DockMenus has different behavior when a currently open (in a zone) window is chosen: it simply activates it in its current zone. A window on another Space of a display with a full-screen window is placed into the targeted zone instead (see **Full-screen pause** in [SPECIFICATION.md](SPECIFICATION.md)). Because a chosen window is activated in place rather than moved, DockMenus chooses a `hasMainWindow` app's main window even while it is in a zone, unlike the Launcher.
...
- **Already-zoned window rows:** Keep DockMenus' activate-in-place behavior; they do not move into the active window's zone. Exception: a window on another Space of a display with a full-screen window is placed like a minimized one, retarget included.
...
- `DockMenus targets zone with active window` toggle in Preferences → Zones → Replacing focused window (default off).
