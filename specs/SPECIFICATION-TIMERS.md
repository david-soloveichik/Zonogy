...
| **Destroyed-window validation retries** | 0.2, 0.4, 0.8, 1.6, 3.2s | `asyncAfter` (backoff) | `ValidationRetryManager.swift` | AX destruction detection is unreliable (some apps never emit destroy notifications). After lifecycle events, a deferred AX-unavailable decision, or full sync deferring a placed native-tab candidate, retries PID-scoped validation to catch closed windows. An AX-invalid window that remains listed in WindowServer stays managed rather than being vacated by these checks. |
...
| **Unmanaged-focus resolution retries** | 0.2, 0.4, 0.8, 1.6, 3.2s | `asyncAfter` (backoff) | `AppController+SystemEvents.swift` | `_AXUIElementGetWindow` can transiently fail, making it unclear if a focused window is managed. Retries to resolve managed/unmanaged classification for hiding zone resize bars (resolving focus also repairs stale full-screen pause state). |
...
| **WinShot full-screen exit wait** | 0.05s poll, 3.0s timeout | `asyncAfter` | `AppController+WinShotFullScreenExit.swift` | An arrangement chosen on a display paused for native full-screen waits for the display to leave full-screen before it opens: pause cleared, full-screen Space gone, and the former full-screen window back on screen (macOS re-presents it ~50ms after the Space is torn down, undoing a minimize issued before then). Events prompt the checks; while any signal is outstanding (or unreadable) they are re-checked every 50ms. If the three are not all met within 3s (e.g. the app never leaves full-screen), the wait is dropped. |
...
| **Activity recording suppression** | 0.5s | deadline-based | `AppController+FloatingZoneIdentity.swift` | Suppresses notification-driven CmdTab/Launcher recency recording and focus-driven relocation from behind full screen during floating zone placement, WinShot restore, and return to a native full-screen Space. Full-screen restoration starts suppression before requesting activation and refreshes the same deadline after the activation call returns, so delayed focus notifications do not bring the app's other windows out from behind full screen. |
| **Pending-prune same-PID discard grace** | 2.0s | deadline-based (lazily checked) | `PendingPrunedWindowStore.swift` | When a new managed `CGWindowID` appears for a PID, pending-prune entries for that PID are discarded only after this grace window. Very recent entries stay restorable because another same-PID window does not prove that a just-pruned `(pid, CGWindowID)` cannot still reappear. |
| **Manual move suppression** | 1.5s | deadline-based | `AppController+SystemEvents.swift` | After Zonogy programmatically moves windows (e.g., display topology change), AX move notifications arrive but are not user-initiated. Suppresses manual-move/drag handling for 1.5s so Zonogy doesn't misinterpret its own moves as user drags. |
| **External-move reassert suppression** | 0.3s | deadline-based (lazily checked) | `WindowController.swift` | After an application moves a managed window itself — a non-programmatic move that is not a recognized Zonogy drag — focus-driven frame reasserts (Sticky Resize restore, ActiveFit reveal/rest) are suppressed for that window for this long so Zonogy does not immediately fight an app-managed move that is still settling. |
...
| Timer | Duration | Mechanism | File | Purpose |
|-------|----------|-----------|------|---------|
| **Deferred minimization debounce** | 0.15s | `DispatchSourceTimer` | `DeferredMinimizationCoordinator.swift` | Shared minimization queue. Used by occlusion/focus-driven floating minimization, the floating-zone explicit-minimize path, and placements that flow through `placeNewWindow` (including arrivals deferred during full screen). The latter queue the displaced occupant rather than minimizing synchronously: a launching app processing its own unminimize queue would otherwise re-unminimize our just-minimized window, creating an infinite ping-pong. Debouncing lets the app drain its queue first. Zonogy-initiated single-window swaps that don't go through `placeNewWindow` (Launcher, drag-drop, moves) still minimize synchronously so any brief visual flash from the AX minimize lands while the incoming window is still hidden. See `SPECIFICATION-IMPLEMENTATION.md` for the flash mental model. |
...
| **Display topology change debounce** | 0.25s | `asyncAfter` | `AppController+SystemEvents.swift` | macOS fires multiple rapid display-change notifications when monitors are connected/disconnected. Waits 250ms for them to settle before recapturing topology. |
...
| **Full-screen check debounce (Space change)** | 0.25s | `asyncAfter` | `AppController+FullScreen.swift` | Some apps don't emit resize events for full-screen windows on Space change. Debounces the rescan to 250ms after the Space change. A return to full screen already queued by window placement is not requested again. Retries deferred arrivals once a display is available, including when a native Space finishes exiting after its pause cleared. |
...
| **Window liveness AX-check cache** | 5.0s | per-window timestamp cache | `WindowController.swift` | The CGWindowList snapshot is the destruction authority in `pruneDestroyedExternalWindows` and runs on every sync. The per-window AX health check (`AXRole` + `AXPosition` reads) detects cached elements that should be rebound; it never vacates a zone while the same window remains in WindowServer. Recently-confirmed-usable elements skip that AX check for 5s, eliminating redundant AX reads during burst syncs. Negative results are not cached, so repair is attempted again on the next sync. The cache is invalidated whenever a window is removed via `removeManagedWindowFromLiveTracking` or its AX element is rebound via `rebindElement`. |
...
| **WinShot occupancy settle delay** | configurable, default 3.0s | `asyncAfter` (per-display `DispatchWorkItem`) | `WinShotOccupancyAutoSaveScheduler.swift` | When WinShot auto-save mode is "on every zone occupancy change", each display's settle timer (re)starts whenever that display's zone occupancy changes; if the arrangement survives the delay unchanged, a snapshot is captured. Because the capture happens a short while after the arrangement settles (while it is still on screen), it doubles as the correct pre-change record for the next change — including externally-initiated ones Zonogy learns of too late to capture directly. Arrangements that don't survive the delay are not saved. (A snapshot that settles while the WinShot chooser is open is saved silently, without refreshing the open chooser.) The configured delay is also reused by the CmdTab just-minimized selection skip (see UX / Interaction). |
...
| **Add-zone indicator hover-exit** | 0.06s | `asyncAfter` | `AddZoneIndicatorManager.swift` | Same hysteresis for the add-zone indicator pill on the display edge. |
| **External drag overlay teardown** | 0.05s | `asyncAfter` | `ExternalZoneDropInterceptor.swift` | Brief pause before cleaning up drag overlay UI and per-gesture state after an external drop, allowing the drop animation to complete; a modifier release inside the pause leaves the cleanup to it. A new drag beginning inside the pause finishes the previous gesture at once, and a drag cancelled with Escape is finished immediately. |
...
| **Edge-pill drop rescue delay** | 0.15s | `asyncAfter` | `AppController+ExternalDrop.swift` | After an external drag is released in an edge pill's display-boundary dead band, waits out the mouse-up/AppKit-delivery ordering skew: if the pill's AppKit drop handler ran, it stamps a recent-drop timestamp (checked with a 0.4s window) and the rescue stands down; otherwise the rescue performs the drop. |
| **External-drag pasteboard poll** | every event for 0.05s, then 0.1s | deadline-based (lazily checked) | `ExternalDragSessionTracker.swift` | Whether another app's drag is in progress can only be learned by reading the drag pasteboard's change count, a round trip to the pasteboard server, and the global drag monitor sees every drag event in every app. While the button is down and the gesture has no recognized drag session yet, every drag or modifier event in the first 50ms after the gesture's first read reads the count again (the source app starts its drag session within the first few drag events), and after that it is read at most every 100ms; once a session is recognized its content cannot change before mouse-up, so polling stops and its acceptability is examined once. Mouse-up (or Escape) records the session as handled. Reads happen only as drag or modifier events arrive, so a drag whose session starts after the first 50ms is recognized at the first event 100ms or more after the previous read; a cursor that stops moving before then is recognized on its next move. |
...
| **CmdTab just-minimized selection skip** | shares the WinShot occupancy settle delay (configurable, default 3.0s) | deadline-based timestamps + modifier-release monitor | `RecentUserMinimizeTracker.swift` | After a user minimize (minimize shortcuts, the zone-navigation Minimize key, the window's own minimize control, app hide), CmdTab's initial selection skips that window for the settle delay — longer while the modifiers held during the minimize are still held, so the skip always covers an in-chord Cmd-M → Cmd-Tab. Sharing the WinShot delay is deliberate: both express how long until the display has settled for the user. Skips also end early when the user selects a window through Zonogy (they have moved on), when the window is restored or destroyed, and when sleep or screen-lock protection begins (modifier releases while locked never reach the monitors, so held gestures are also reconciled against the live modifier state whenever the skips are queried). |
| **Paired-shortcut hold follow-up** | 0.5s | `asyncAfter` (cancelable work item) | `AppController+ShortcutHold.swift`, `ShortcutHoldPolicy.swift` | The clear/reset and minimize/remove shortcuts perform their first action on press, then perform the paired action if the chord remains held for 0.5 seconds. Releasing any part of the chord or changing zone topology cancels the follow-up. |
| **Held-shortcut stale-mark recovery** | 3.0s | event-timestamp check | `HotkeyService.swift` | A hold-capable shortcut suppresses repeat press events until its release arrives. If that release is lost, a press more than 3 seconds after the preceding press is treated as fresh so the shortcut cannot remain disabled indefinitely. |
| **Pass-through re-check after clicks** | 0.25s after mouse-up | `asyncAfter` | `AppController+WindowLifecycle.swift`, `AppController+PlaceholderPassThrough.swift` | A click can reorder windows relative to a placeholder without any focus or sync event: releasing a press on the placeholder leaves its panel newly raised, and while pass-through holes are active a click in another app can raise the window under a hole or hit a stale region (while the Launcher is open, the same re-check lets it yield to a window that appeared beneath it silently). The window server applies click-raises asynchronously and no event marks their completion, so both paths re-check pass-through regions shortly after the release. (The other pass-through triggers need only the shared scheduler's next-runloop delay, since Zonogy issues their ordering changes itself.) |
| **Desktop change → pass-through refresh debounce** | FSEvents latency 0.25s; debounce 0.5s, follow-up +1.0s | FSEvents + `asyncAfter` (work item) | `DesktopChangeWatchService.swift` | Desktop file events (icons added/removed/renamed, and Finder's `.DS_Store` icon-position writes, which land at drag-drop time), volume mounts, Finder relaunches, and display changes alter the desktop icons under placeholders without any click or focus event. Debounces event bursts and gives Finder a moment to place or reflow icons (or rebuild its hierarchy) before pass-through holes are recomputed; a single follow-up refresh one second later covers icons Finder places slowly. |
| **Desktop icon frames freshness window** | 5.0s | deadline-based | `AppController+PlaceholderPassThrough.swift` | Icon-frame invalidation is event-driven: desktop-change notifications and display changes clear the freshness stamp. Clicks therefore reuse the cached frames, so the steady-state post-click re-check performs no Finder AX work. Expiry does not schedule anything; it only caps how long a later refresh may reuse the cache, bounding the staleness of any reflow no event catches, which self-heals on the click after the stale one. |
...
| Timer | Duration | Mechanism | File | Purpose |
|-------|----------|-----------|------|---------|
| **Wake readiness polling** | 0.5s repeating with 0.25s leeway | `DispatchSourceTimer` | `AppController+SleepWake.swift` | After `screensDidWakeNotification` or `loginwindow` stops being active, AX APIs may not be ready yet. Polls every 0.5s until the display is awake, the session is unlocked, and `NSWorkspace.shared.frontmostApplication` is non-nil and is not `loginwindow`. The timer uses leeway so macOS can coalesce wakeups during recovery. (Uses NSWorkspace instead of AX because AX can hang indefinitely with some apps during wake recovery.) |
...
When displays go to sleep (`screensDidSleepNotification`) or `loginwindow` becomes active, Zonogy prevents AX operations from firing during the protected interval using two strategies:

**Explicitly cancelled:** Validation retries, wake readiness timer, frame retries, capture retries, display-change recapture timers, and display-change debounce timer are actively cancelled when sleep/wake protection begins.

**Guard-and-bail:** Some delayed work items are not explicitly cancelled but instead check `sleepWakeProtectionActive` when they fire and return early if true. This includes: minimize verification (`AppController+ZoneLifecycle.swift`) and startup capture (`AppController+Startup.swift`).
