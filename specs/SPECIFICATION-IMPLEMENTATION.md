...
## Directional Navigation Selection

Zone navigation selects structurally within a display; across displays, geometry picks the display and the entry into it is structural.

On one display, moves are read from the zone model — each tiling zone's column side and stack row, with the floating zone bar as the bottom-most stop below both columns. Crossing columns lands in the matching row: top to top, bottom to bottom, and a full-height column enters a stack at the stack's top zone. Off the floating zone bar, up climbs to the zone nearest the bar (a stacked bottom zone before a full-height column, then the lower zone index) and a horizontal press lands on the bottom-most zone of the column in that direction.

A press past the edge of the layout crosses to another display. Each display lies in exactly one direction from the current one: with overlapping x-ranges it is above or below, with overlapping y-ranges left or right. The press moves to the nearest display lying in the pressed direction (by center distance along that axis, then across it) and enters through its near side: from below, the floating zone bar; from above, the zone nearest the top edge (a stacked top zone before a full-height column); from the side, the near column at the matching row, with a bar entering at that column's bottom-most zone.

Each selection carries the trail of moves that produced it. A press exactly opposite to the trail's last move pops back to that move's source, so any path retraces step by step to the gesture's start. (Note that moves are not symmetric — a full-height column reached from a stack's bottom re-enters the stack at its top — which is why reversal is remembered rather than recomputed.) A jump starts a fresh trail.

A display key finds its display's last-used window in the same recency order as CmdTab and the Launcher. Because commits happen on modifier release, the gesture runs through a keyboard event tap rather than ordinary hotkeys. Jump keys ignore auto-repeat (a held cell key could otherwise keep adding zones), and a key the gesture takes stays taken until it is released, so a key still held when the gesture ends neither repeats into the focused app nor fires the shortcut behind it. The held modifier combination is configurable (at least two modifiers, like the mouse-gesture modifiers). The in-gesture action keys are borrowed from their shortcuts' bindings (Move Focused Window to Target Zone, Show Launcher, Add Zone, Remove Zone, Minimize Focused Window). The gesture snapshots the zones when it engages; commits re-check live occupancy. Anything that could invalidate the snapshot mid-gesture (another Zonogy shortcut firing, or a display change) cancels the gesture rather than letting a commit act on stale zones (except, of course, a zone the gesture itself adds, around which the gesture is rebuilt).

## Shortcut Conflicts

Overlapping shortcuts are shown, not prevented (as System Settings does). A shared chord only ever reaches one of its holders: an event tap that engages on it (Zone Navigation, CmdTab) swallows it before any hotkey fires, and duplicate hotkeys cannot both fire. Overlaps count Zone Navigation's chords (its modifiers plus its navigation keys) and a CmdTab shortcut's Shift variant, which cycles backward.
...
- The CmdTab and Zone Navigation keyboard event taps must decide whether to consume each keystroke before the system delivers it to the frontmost application, so the system waits on the tap for every keystroke in every app. The taps are therefore serviced on a dedicated thread that answers at once, rather than the main run loop, where the wait would include Zonogy's Accessibility stalls. A keystroke that is Zonogy's is passed to the main thread, which processes it when available.
- The zone click tap likewise answers off the main thread, on a mouse tap thread separate from the keyboard taps' because its answer can involve a wait. A click can only be Zonogy's while CmdTab is showing or the gesture modifiers are held; every other click, which is nearly all of them, passes at once without involving the main thread. A click that may be Zonogy's is decided on the main thread, and the tap thread waits for the answer, because it depends on where the click landed: zone frames, zone bar hit areas, and Zonogy's own window frames live there.
- The Dock press tap answers on the mouse tap thread as well: a click outside the Dock frame passes at once, and only a click inside it waits for the main thread. The Dock taps are also enabled only while a click or drag could concern Zonogy; see the Click Interception and Dock Icon Drag Interception notes in `SPECIFICATION-DOCKMENUS.md`.
...
When applying a target frame, the placement code first chooses a position-vs-size order so the in-progress frame stays inside the visible display, applies it, and reads back the resulting frame to verify. If the first pass produced the target frame, the move is done. Otherwise, it follows up with the opposite order as a recovery step, falling through to the existing retry chain if needed.
...
- **(Per-application) destroyed-window validation retries (PID-scoped):** After AX-relevant lifecycle events (window focus changes, application activation/deactivation/hide), we schedule a short series of PID-scoped validation passes (≈0.2–3.2s backoff) when window liveness is inconclusive (see "Destroyed Window Detection" above). These retries are cancelled when the process exits, when all windows are pruned, or when displays go to sleep (`handleScreensDidSleep` calls `cancelAllValidationRetries()`).
...
- **(Per-window) AX frame application retries:** When Zonogy moves/resizes a window to its zone frame, `WindowController` retries with backoff (≈0.25–3.0s, a few attempts) until the origin is at the target and the app accepted the size write — i.e. `AXUIElementSetAttributeValue(…, kAXSizeAttribute, …)` returned `.success`. The return status tells a transient failure from a permanent one: a non-`.success` means the app refused the resize (e.g. a just-created window not yet ready) — keep retrying; a `.success` that still doesn't reach the requested size is a real size constraint (e.g. a min-width window) — settle immediately and let ActiveFit handle any overflow. Retry chains are cancelled when zone geometry changes, displays go to sleep, or a new `moveWindow` call supersedes the old target. **ActiveFit interaction:** When the retry chain settles, `WindowController` notifies `AppController` via `frameRetryDidSettle`, which evaluates whether ActiveFit reveal mode is needed for zone 2/3 windows.
- **(Per-application) unmanaged-focus resolution retries (PID-scoped):** The stored unmanaged-focus state drives one behavior — hiding zone resize bars on the focused display, so Zonogy does not draw interactive chrome over an unrelated focused window. The classifier is tri-state (`managed`, `confirmed unmanaged`, or `unresolved`) and only confirmed-unmanaged focus suppresses UI. Focus is confirmed unmanaged if it fails any of the application or window-level management rules (see `SPECIFICATION.md`) other than the `_AXUIElementGetWindow` check, whose failure can be transient. Unresolved focus (e.g., transient `_AXUIElementGetWindow` failures) is treated as managed for suppression purposes and retried with PID-scoped exponential backoff (≈0.2–3.2s); retries are cancelled when focus resolves, the frontmost PID changes, or displays sleep. Zonogy's own Preferences window also suppresses resize bars on its display (resolved synchronously via AppKit rather than through this retry pipeline).
...
On displays placed above or left of the primary display (negative global coordinates), the system pins a hard-slammed cursor essentially on the display's boundary coordinate. Drag-session hit-testing resolves that position to the row just past the display, which no window's drag region can cover (regions are clipped to the visible display area) — so a fast external drag to such an edge never reaches the edge bar's drop handlers, no matter how the bar's window is framed.
...
- **Hit overhang:** each edge bar's hit rectangle invisibly extends a couple of points past its display edge. This serves Zonogy's own cursor tests: targeting when a managed window is dragged onto the bar (which has no nudge), and the external-drag tracking above — the tracking must first recognize a pinned cursor as being on the bar before it can highlight, nudge, or drop, and a rectangle ending flush at the edge would not contain a cursor pinned exactly on the boundary coordinate. The overhang applies only where the space past the edge is empty, never where another display adjoins (there the cursor travels through instead of pinning).
