...
- Bug report: Dragging a tiled window's last native tab into another tiled window can leave the source tab assigned to its old zone, so later focus/layout handling can snap the visible tab back there instead of treating the drag as a merge. Some apps report this final frame change as `AXResized`, not `AXMoved`.
  - Think about: When an application-driven move or resize puts a placed same-process source window at another placed window's frame, collapse the source into the destination: destination keeps its `windowId` and zone while adopting the source backing, and the source zone is emptied. Suppress immediate focus-driven reasserts briefly while such app-driven frame changes settle.
...
- Bug report: After sleep/wake or display-change recapture, a pruned window can be re-placed from stale recapture state, leaving a tiling zone falsely occupied and routing subsequent windows into the floating zone.
...
- Bug report: After wake, recapture can place a stale tracked-but-unzoned native-tab identity into a new zone even though it was not seen live during that recapture, making the visible tab jump while the old zone remains falsely occupied.
  - Think about: Recapture should only place unzoned candidates that the same recapture pass revalidated as live with a WindowServer frame.
...
- Bug report: A managed window can lose its zone and appear behind a placeholder while the real window remains open. This can follow a spurious `AXUIElementDestroyed` (for example, Finder recycling an element during rename) or a failed AX liveness read while the login screen is active (for example, pressing the laptop power button). Background windows are especially vulnerable because only the returning frontmost app receives an immediate activation capture.
  - Think about: The lifetime of an AX element is not always the lifetime of its window, and AX can be unreliable during sleep/wake.

- Bug report: Some applications leave a closed window registered with WindowServer, so DockMenus continues listing an orphan.
  - Think about: WindowServer and AX can disagree about liveness: WindowServer may retain a closed window, while an AX failure may be transient.
...
- Bug report: Dragging a managed window across a display boundary can make macOS resize the window itself (e.g. clamping an oversized window to fit the display it is crossing into), firing a stream of non-programmatic AXResized events. Handling these as user "manual resize ended" corrupts manual-resize/Sticky-Resize state and thrashes the resize bars for the whole drag.
...
- Bug report: Fast external drags slammed to the bottom edge of a display sitting above/left of the primary display (negative global coordinates) missed the floating zone bar entirely — no expansion, no drop — while the same gesture worked on the primary display.
  - Think about: AppKit drag delivery structurally misses a cursor pinned on a display boundary (drag regions are clipped to the on-screen area), so the edge pills need app-side hover/drop handling keyed off precise cursor positions. Prefer nudging a pinned drag back inside the bar so the system delivers the drop itself (a synthesized drop leaves the drag image flying back to its origin); a synthesized drop must stay a last resort that fires only where AppKit provably cannot deliver, and never for a cancelled drag session. Hit areas may overhang a display edge only into void, never into an adjoining display.
