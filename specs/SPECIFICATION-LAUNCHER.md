...
2. **If selecting a running app with 1+ managed windows:**
   ...
   - **Main window apps** (`hasMainWindow: true` in config): selects the main window (lowest `CGWindowID`), unless it is already placed in a zone; then selects as for other apps
...
- **Targeting preference:** `Launcher keyboard shortcut targets zone with active window` in Preferences → Zones → Replacing focused window (default off)
...
- **Manual refresh fallback:** Opening Preferences forces an immediate rescan of launcher items, reloads `launcher-config.json` (for alias changes), and reloads all `config.json` fields (exceptions, ignored bundle identifiers, `deriveBundleIdFromPathForProcesses`), using the same in-place update behavior for the launcher list.
