...
- IMPORTANT: For codex only (not claude): If running `swift build` or other commands gives a sandbox error, ask me for approval to run with elevated privileges. Also if you encounter path errors, the path should be correct in zsh (rather than bash). Similarly for accessibility API permission errors.
