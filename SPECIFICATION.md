...
On a screen, presses follow the zone layout: up and down walk a column, with the floating zone bar as every screen's bottom-most stop; left and right cross between the columns, staying in the same row. A press past the edge of the layout crosses to the next screen in that direction; entering a screen from below stops at its floating zone bar first.

Since our navigation rules are in general not symmetric, we record the navigation path during a gesture session; then navigating in the opposite order follows the original path backward.

The gesture starts from the focused managed window's zone, and the first press moves off it. When the Launcher is open, or no managed window is focused, the gesture starts from the targeted zone instead (the Launcher anchors to the target, so this is the Launcher's zone): if that zone is filled, the first press selects it in place. A first press with no zone in the pressed direction selects the start zone in place, so the circle always appears.
