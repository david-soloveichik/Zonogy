...
The circle navigates every zone on every screen (skipping screens paused for full-screen), staying put when nothing lies in the pressed direction. On a screen, presses follow the zone layout: up and down walk a column, with the floating zone bar as every screen's bottom-most stop; left and right cross between the columns, staying in the same row. A press past the edge of the layout crosses to the next screen in that geometric direction; entering a screen from below stops at its floating zone bar first. Pressing the opposite of the press that moved the circle somewhere backs it out to where it came from (step by step, all the way to where the gesture began). If a tiling zone is selected, the circle centers on it. If a floating zone is selected, the upper half of the circle is shown resting on the screen's bottom edge (at the floating zone bar). The selection rules are specified in [SPECIFICATION-IMPLEMENTATION.md](SPECIFICATION-IMPLEMENTATION.md).

The gesture starts from the focused managed window's zone, and the first press moves off it. When the Launcher is open, or no managed window is focused, the gesture starts from the targeted zone instead (the Launcher anchors to the target, so this is the Launcher's zone): if that zone is filled, the first press selects it in place — so tapping any arrow and releasing focuses the targeted zone's window. A first press with no zone in the pressed direction selects the start zone in place, so the circle always appears.
