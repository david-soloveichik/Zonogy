...
- Control-Command with an arrow key (Up, Down, Left, or Right) moves the target to the nearest zone in that physical direction. All targetable zones are considered together: the tiling zones on every screen plus each screen's floating zone, each placed by its true on-screen rectangle. The floating zone sits at the location of its bottom-edge bar.
