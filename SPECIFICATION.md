...
3. If the moved source window and the destination placed window have coincident position and width, Zonogy treats this as a tab merge: the destination managed window adopts the source backing and keeps its zone, while the source managed window is removed from its old zone so that zone shows its placeholder. (While an application-driven move is still settling, Zonogy also avoids immediately pulling the focused window back to its zone.)
