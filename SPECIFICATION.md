...
**Full-screen pause:** A native macOS full-screen window pauses Zonogy on its display. Zonogy treats full screen as a temporary state of the display rather than as a second desktop: although technically it's another macOS Space, we want the user's mental model to exclude Spaces when using Zonogy. Accordingly, a managed window that is not currently visible on any display is "effectively minimized" in the mental model. Another Space on the same display as the full-screen window is never shown: if switched to it (including even via a deliberate user-initiated Spaces swipe), the display returns to full screen. (When every display is full-screen, there is no visible zone to receive a window, so selecting a window on another Space does nothing.) No Zonogy UI/overlays should appear there (placeholders, Launcher, targeting indicators, add‑zone indicator, zone resize bars, drag overlays, etc). The WinShot chooser is the one exception — choosing an arrangement there takes the display out of full screen (see [SPECIFICATION-WINSHOT.md](SPECIFICATION-WINSHOT.md)). Zones on that display are not targetable; when a display enters full‑screen mode, retarget using normal rules. (If all displays are full‑screen, target display 0 (normal rules) as a last resort to maintain the invariant that one zone is always targeted.)

When a display is full-screen and (another Space of) it has a window opened, unminimized, or focused by something outside Zonogy:
