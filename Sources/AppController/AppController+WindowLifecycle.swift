...
extension AppController {
    ...
    func handleDestroyedWindows(
        ...
        for windowId in windowIds {
            ...
            performWindowLayoutRemovalPreSyncCleanup(
                ...
                reason: reason,
                retarget: retarget,
                notifyFullScreenTrackerOfClose: true,
                frontmostReplacementWindowId: nil,
                frontmostReason: "window-destroyed:\(windowId)",
                sizeClearReason: "destroyed-window"
            )
        }
        ...
        for windowId in windowIds {
            performWindowLayoutRemovalPostSyncCleanup(windowId: windowId, reason: "close")
        }
    }

    private func performWindowLayoutRemovalPreSyncCleanup(
        windowId: Int,
        reason: String,
        retarget: Bool,
        notifyFullScreenTrackerOfClose: Bool,
        frontmostReplacementWindowId: Int?,
        frontmostReason: String,
        sizeClearReason: String
    ) {
        if let workItem = fullScreenCheckWorkItemsByWindowId.removeValue(forKey: windowId) {
            workItem.cancel()
        }

        if notifyFullScreenTrackerOfClose {
        ...
        if currentFrontmostManagedWindowId == windowId {
            setCurrentFrontmostManagedWindowId(frontmostReplacementWindowId, reason: frontmostReason)
        }

        // If the window is staged for deferred prune, keep its remembered sizes alive — a
        // false-positive detection followed by restore would otherwise lose user state. The sizes
        // are cleared when the pending-prune entry is permanently discarded (see
        // windowController(_:didDiscardPendingPrunedWindowIds:reason:)).
        if !windowController.hasPendingPrunedEntry(forWindowId: windowId) {
            _ = clearRememberedManualResizeSize(for: windowId, reason: sizeClearReason)
    ...
    private func performWindowLayoutRemovalPostSyncCleanup(windowId: Int, reason: String) {
    ...
    func windowFocusChanged(pid: pid_t, focusedWindowId: Int?) {
        ...
        // If the focused window was just moved by its own application, skip immediate
        // focus-driven frame reasserts (Sticky Resize restore, ActiveFit reveal/rest).
        // Repositioning it right away can fight app-managed moves that are still settling.
        if let focusedWindowId, windowController.isExternallyMovingRecently(windowId: focusedWindowId) {
            Logger.debug("Skipping focus-driven frame reasserts for window \(focusedWindowId); moved externally")
        } else {
    ...
    func placeholderPressEnded() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
            self?.schedulePlaceholderPassThroughRefresh(reason: "placeholder-press-ended")
        }
    }
    ...
    func windowController(
        _ controller: WindowController,
        didCollapseNativeTabSourceWindow sourceWindowId: Int,
        into destinationWindowId: Int
    ) {
        performWindowLayoutRemovalPreSyncCleanup(
            windowId: sourceWindowId,
            reason: "native-tab-source-collapse",
            retarget: true,
            notifyFullScreenTrackerOfClose: false,
            frontmostReplacementWindowId: destinationWindowId,
            frontmostReason: "native-tab-source-collapse",
            sizeClearReason: "native-tab-source-collapse"
        )
        ...
        syncWindowsToZones()
        performWindowLayoutRemovalPostSyncCleanup(windowId: sourceWindowId, reason: "native-tab-source-collapse")
    }
    ...
    func windowManualResizeDidEnd(windowId: Int, screenId: CGDirectDisplayID?, frame: CGRect) {
        ...
        if let resolvedScreenId {
            ...
            Logger.debug("Window \(windowId) manual resize ended in zone \(zoneIndex) on screen \(screenIndex); deferring snapback until another managed window on this display becomes active or its zones change")
        } else {
            Logger.debug("Window \(windowId) manual resize ended in zone \(zoneIndex) on unknown screen; deferring snapback until another managed window on this display becomes active or its zones change")
        }
    }
    ...
    func windowManualMoveDidEnd(windowId: Int, finalFrame: CGRect) {
        ...
        // Only an activated drag displaces the window; a cancelled drag candidate (e.g. the small
        // origin shift of a left- or top-edge resize) leaves it where the user put it.
        let wasDragging = dragDropCoordinator.currentDragWindowId == windowId
        ...
        // A drag displaced the window, so any manual-resize detachment ends here: a resolved drop
        // places it afresh, and after a cancelled drag the sync returns it to its zone frame.
        if wasDragging {
            manualResizeDetachedWindowIds.remove(windowId)
        }
        syncWindowsToZones()
...
extension AppController {
    ...
    internal func handleManualResizeActivationCandidate(pid: pid_t?) {
        // No frontmost application: like ActiveFit, leave detached windows as they are.
        guard let pid else {
            return
        }
        handleManualResizeFocusChange(pid: pid)
    ...
    /// becoming active returns the other detached windows on its display to their zone frame, while
    /// focus on Zonogy itself or on an unmanaged/untracked window leaves detached windows at their
    /// custom size. Displays are independent, so focus changes on one display never disturb detached
    /// windows on another.
    internal func handleManualResizeFocusChange(pid: pid_t) {
        ...
        guard pid != getpid(),
              ...
              isLayoutManagedWindow(focused),
              let screenId = focused.screenDisplayId ?? detectScreenId(for: focused) else {
            return
        }

        // A managed layout window became active: snap the other detached windows on its display
        // back to their zone frame, keeping the now-active window at its custom size.
        let candidateIds = manualResizeDetachedWindowIds.filter { windowId in
            windowId != focused.windowId && windowController.window(withId: windowId)?.screenDisplayId == screenId
        }
        Logger.debug("Manual resize focus change for pid \(pid) (focused: \(focused.windowId), candidates on its display: \(candidateIds.count))")
    ...
    internal func snapManuallyResizedWindowBackToZoneIfNeeded(windowId: Int, reason: String) {
        ...
        // another managed window on its display (it deliberately keeps reveal on focus to an
        // unmanaged window).
        if activeFitStates[windowId] != nil {
            return
        }
