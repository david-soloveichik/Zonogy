...
private enum KeyFitPolicyTests {
    @discardableResult
    static func run() -> Bool {
        var allPassed = true
        ...
        func assert(_ condition: @autoclosure () -> Bool, _ message: String) {
            if !condition() {
                print("KeyFitPolicyTests: \(message)")
                allPassed = false
            }
        }
        ...
        if let frame = KeyFitPolicy.revealFrameIfNeeded(
            zoneIndex: 2,
            ...
            assert(false, "expected KeyFit to translate horizontally for oversized width in zone 2")
        }
        ...
        if let frame = KeyFitPolicy.revealFrameIfNeeded(
            ...
            assert(false, "expected KeyFit to translate for combined width/height overflow in zone 3")
        }

        let noOverflow = KeyFitPolicy.revealFrameIfNeeded(
            zoneIndex: 2,
        ...
        assert(noOverflow == nil, "KeyFit should not trigger when the frame already fits")

        let zoneOne = KeyFitPolicy.revealFrameIfNeeded(
            zoneIndex: 1,
        ...
        assert(zoneOne == nil, "KeyFit should ignore zone 1 even if it would overflow")

        let tinyOverflow = KeyFitPolicy.revealFrameIfNeeded(
            zoneIndex: 2,
        ...
        assert(tinyOverflow == nil, "KeyFit should ignore sub-tolerance overflow")

        if allPassed {
            print("KeyFitPolicyTests: all tests passed")
        }
        return allPassed
    }
}
