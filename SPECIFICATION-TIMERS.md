...
| **Follows-focus activation settlement** | 0.6s | `asyncAfter` | `AppController+FocusFollowActivationSettlement.swift` | After an app activates in follows-focus mode, briefly delay retargeting to that app's existing focused window if it would steal the current target. This gives a newly opening window or document time to appear and be placed into the preserved target. If no new managed window appears and the target did not otherwise change, retarget when the timer ends. |
| **Empty-zone retarget protection** | 0.5s | deadline-based | `AppController+ZoneTargeting.swift` | When a tiled window is closed/minimized and its zone retargets to the now-empty zone, protects that retarget from being overridden by the OS auto-focusing the app's expected sibling fallback window (follows-focus mode only). Suppresses only that specific fallback window's retarget while the current target is still the protected zone and the deadline hasn't passed. Cleared early if the target changes away from the protected zone. |
