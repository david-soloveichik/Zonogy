...
| **Dock observer re-establish debounce** | 0.5s | `asyncAfter` | `DockAXNotificationMonitor.swift` | The Dock can rebuild its accessibility hierarchy in place (wake, display reconfiguration), firing `AXUIElementDestroyed` on observed elements in bursts. Coalesces them into a single re-discovery + re-registration pass so Dock hover detection rebinds to the new AXList. |
