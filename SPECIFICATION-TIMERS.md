...
When screens go to sleep (`screensDidSleepNotification`) or `loginwindow` becomes active, Zonogy prevents AX operations from firing during the protected interval using two strategies:

**Explicitly cancelled:** Validation retries, wake readiness timer, frame retries, capture retries, screen-change recapture timers, and screen-change debounce timer are actively cancelled when sleep/wake protection begins.
