...
| **Window capture retries** | 1.0, 2.0, 4.0, 8.0s | `asyncAfter` (backoff) | `WindowCapturePipeline.swift` | `AXWindowCreated` sometimes fires before the window is queryable. With native macOS tab handling enabled, `_AXUIElementGetWindow` can also expose a fresh `CGWindowID` before `CGWindowListCopyWindowInfo` returns its live frame. Retries capture with exponential backoff until a manageable window is found. |
