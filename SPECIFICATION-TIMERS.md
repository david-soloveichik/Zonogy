...
| Timer | Duration | Mechanism | File | Purpose |
|-------|----------|-----------|------|---------|
| **Deferred minimization debounce** | 0.15s | `DispatchSourceTimer` | `DeferredMinimizationCoordinator.swift` | Shared minimization queue used by occlusion/focus-driven floating minimization and the floating-zone explicit-minimize path. Tiled-zone and floating-zone *placement* displacements bypass this queue and minimize the displaced occupant synchronously, so AX kAXMinimized's brief flash-to-key on the displaced window happens before the incoming window is positioned/raised/revealed (avoiding a visible flicker over the new occupant — applies whether the incoming window is being unminimized or moved while already visible). |
